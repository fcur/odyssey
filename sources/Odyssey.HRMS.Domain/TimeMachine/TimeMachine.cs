...
public sealed class TimeMachine
{
    ...
    public static Result<TimeMachine, DomainError> Create(Employee employee,
        IReadOnlyCollection<TimeOffRequest> timeOffRequests)
    {
        ...
        {
            return Result.Failure<TimeMachine, DomainError>(TimeMachineDomainError.InvalidStartDate(employee.StartDate.Value));
        }

        return new TimeMachine(employee, timeOffRequests);
    }

    public Result<TimeMachineState, DomainError> GoTo(DateTimeOffset atTime)
    {
