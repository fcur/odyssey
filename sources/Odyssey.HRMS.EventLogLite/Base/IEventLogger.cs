...
{
    Task<PositionPair>  WriteBatch<TEvent>(IReadOnlyCollection<LogMessage<TEvent>> logMessages, TSegment segment, CancellationToken cancellationToken = default) where TEvent : class;
    ...
    Task<LogMessage<TEvent>?> ReadLast<TEvent>(TSegment segment, CancellationToken cancellationToken = default) where TEvent : class;
